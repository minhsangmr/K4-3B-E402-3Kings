...
Trang tĩnh, không cần build. Mở [`index.html`](index.html) trực tiếp trong trình duyệt là dùng được tab ①–④. Riêng tab ⑤ Eval cần nạp `eval/golden_set.json` nên chạy qua localhost:

```bash
python3 -m http.server 8000
```

rồi mở `http://localhost:8000/codebase/`.
